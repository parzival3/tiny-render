  ...
  while (curpix < npixels) {
    ...
    while (curpix + run_length < npixels && run_length < max_chunk_length) {
      ...
      for (int t = 0; succ_eq && t < bpp; t++)
        succ_eq = (data[curbyte + static_cast<size_t>(t)] == data[curbyte + static_cast<size_t>(t) + static_cast<size_t>(bpp)]);
    ...
    curpix += run_length;
    out.put(raw ? static_cast<char>(run_length - 1) : static_cast<char>(run_length + 127));
    if (!out.good()) {
      std::cerr << "can't dump the tga file\n";
      return false;
    }
