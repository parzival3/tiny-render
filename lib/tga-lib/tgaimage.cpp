...
TGAImage::TGAImage(const int w_, const int h_, const int bpp_)
    : w(w_), h(h_), bpp(bpp_), data(static_cast<size_t>(w_ * h_ * bpp_), 0) {}
...
bool TGAImage::read_tga_file(const std::string filename) {
    ...
    if (3 == header.datatypecode || 2 == header.datatypecode) {
        in.read(reinterpret_cast<char *>(data.data()), static_cast<long>(nbytes));
        if (!in.good()) {
            in.close();
...
bool TGAImage::load_rle_data(std::ifstream &in) {
    ...
    do {
        ...
        chunkheader = static_cast<std::uint8_t>(in.get());
        if (!in.good()) {
            std::cerr << "an error occured while reading the data\n";
            return false;
        }
    ...
    while (curpix < npixels) {
        ...
        while (curpix + run_length < npixels && run_length < max_chunk_length) {
            ...
            for (int t = 0; succ_eq && t < bpp; t++)
                succ_eq =
                    (data[curbyte + static_cast<size_t>(t)] ==
                     data[curbyte + static_cast<size_t>(t) + static_cast<size_t>(bpp)]);
            curbyte += static_cast<size_t>(bpp);
        ...
        curpix += run_length;
        out.put(raw ? static_cast<char>(run_length - 1)
                    : static_cast<char>(run_length + 127));
        if (!out.good()) {
            std::cerr << "can't dump the tga file\n";
            return false;
        }
...
TGAColor TGAImage::get(const int x, const int y) const {
    ...
    return TGAColor(data.data() + (x + y * w) * bpp, static_cast<std::uint8_t>(bpp));
}

void TGAImage::set(int x, int y, const TGAColor &c) {
    if (!data.size() || x < 0 || y < 0 || x >= w || y >= h) return;
    memcpy(data.data() + (x + y * w) * bpp, c.bgra, static_cast<size_t>(bpp));
}
...
void TGAImage::flip_horizontally() {
    ...
    for (int i = 0; i < half; i++)
        for (int j = 0; j < h; j++)
            for (int b = 0; b < bpp; b++)
                std::swap(data[static_cast<size_t>((i + j * w) * bpp + b)],
                          data[static_cast<size_t>(((w - 1 - i + j * w) * bpp + b))]);
}
...
void TGAImage::flip_vertically() {
    ...
    for (int i = 0; i < w; i++)
        for (int j = 0; j < half; j++)
            for (int b = 0; b < bpp; b++)
                std::swap(data[static_cast<size_t>((i + j * w) * bpp + b)],
                          data[static_cast<size_t>((i + (h - 1 - j) * w) * bpp + b)]);
}

size_t TGAImage::width() const { return static_cast<size_t>(w); }

size_t TGAImage::height() const { return static_cast<size_t>(h); }
